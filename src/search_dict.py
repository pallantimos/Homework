...
def process_bank_search(data:list[dict], search:str) -> list[dict]:
    ...
    new_data = [x for x in data if x and re.search(re.escape(search), x['description'].lower(), flags=re.I)]
...
def process_bank_operations(data:list[dict], categories:list)->dict:
    ...
    data_description = [d.get('description') for d in data if d.get('description') not in categories]
